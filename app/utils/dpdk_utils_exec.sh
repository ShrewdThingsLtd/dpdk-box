...
exec_install_config_editor() {
	...
	local config_dir=${SRC_DIR}/config
	mkdir -p ${config_dir}
	pushd ${config_dir}
	bower --allow-root --no-interactive install -g jQuery#3.3.1
	bower --allow-root --no-interactive install angular-ui-sortable
	bower --allow-root --no-interactive install bootstrap
	bower --allow-root --no-interactive install angular-schema-form angular-schema-form-bootstrap
	bower --allow-root --no-interactive install angular-schema-form angular-schema-form-bootstrap
	#cd ${config_dir}/bower_components/angular-schema-form
	#bower --allow-root install
	pushd ${config_dir}/bower_components/angular-schema-form-bootstrap
	bower --allow-root --no-interactive install
	pushd ${config_dir}/bower_components/angular-schema-form-bootstrap/bower_components/bootstrap/dist/css
